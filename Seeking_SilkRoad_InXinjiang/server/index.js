...
const db = require('./db')
const { ai, jwtSecret, port } = require('./config')
const { runTravelPlanningAgent } = require('./ai-rag')
...
app.get('/api/destinations', asyncRoute(async (req, res) => {
  ...
  res.json({ list: result.rows.map((row) => mapDestination(req, row)) })
}))
...
app.get('/api/destinations/:id', asyncRoute(async (req, res) => {
  ...
  res.json({ data: mapDestination(req, result.rows[0]) })
}))
...
app.post('/api/ai/travel-plan', asyncRoute(async (req, res) => {
  try {
    const result = await runTravelPlanningAgent({
      db,
      aiConfig: ai,
      messages: Array.isArray(req.body.messages) ? req.body.messages : [],
      preferences: req.body.preferences && typeof req.body.preferences === 'object' ? req.body.preferences : {},
      context: normalizeText(req.body.context),
    })
    ...
    res.status(statusCode >= 400 && statusCode < 600 ? statusCode : 500).json({
      message: error.message || 'AI 行程规划失败，请稍后再试。',
    })
  }
}))
