      ...
      <view class="hero-gradient assistant-banner">
        <view class="hero-inner section">
          ...
          <view class="hero-status">
            ...
            <text class="hero-status-text">{{ hasApiKey ? '后端规划服务已接入' : '内部配置未填写' }}</text>
          </view>
        </view>
      </view>
          ...
          <view v-else class="message-list">
            ...
            >
              ...
              <view class="message-bubble" :class="item.role === 'user' ? 'user-bubble' : 'assistant-bubble'">
                ...
                <view v-if="getMessageSources(item).length" class="source-panel">
                  <text class="source-title">参考资料</text>
                  <view class="source-list">
                    <view
                      v-for="source in getMessageSources(item)"
                      :key="`${item.id}-${source.sourceType}-${source.id}`"
                      class="source-item"
                      :class="{ tappable: canOpenSource(source) }"
                      @tap="openSource(source)"
                    >
                      <view class="source-head">
                        <text class="source-badge">{{ getSourceTypeLabel(source) }}</text>
                        <text class="source-name">{{ source.title }}</text>
                      </view>
                      <text class="source-snippet">{{ source.snippet }}</text>
                    </view>
                  </view>
                </view>
              </view>
        ...
        <view class="composer-input-shell">
          ...
          <view class="composer-foot">
            <view class="composer-status-pill" :class="{ offline: !hasApiKey }">
              ...
              <text class="composer-hint">{{ hasApiKey ? 'RAG 规划在线' : '等待后端配置' }}</text>
            </view>
...
import { AI_MESSAGE_STORAGE, clearAiMessages } from '../../config/ai'
import {
...
function getMessageSources(item) {
  if (!item || item.role !== 'assistant' || item.isStreaming) {
    return []
  }

  return Array.isArray(item.sources) ? item.sources.filter((source) => source?.title).slice(0, 6) : []
}

function getSourceTypeLabel(source) {
  return source?.sourceType === 'destination' ? '目的地' : '攻略'
}

function canOpenSource(source) {
  return Boolean(source?.sourceType === 'destination' && source.destinationId)
    || Boolean(source?.sourceType === 'guide' && source.guideSlug)
}
...
function openSource(source) {
  ...
  if (source.sourceType === 'destination') {
    uni.navigateTo({ url: `/pages/destination-detail/index?id=${encodeURIComponent(source.destinationId)}` })
    return
  }

  uni.navigateTo({ url: `/pages/guide-detail/index?id=${encodeURIComponent(source.guideSlug)}` })
}
...
async function sendQuestion(question, extraContext = '') {
  ...
  try {
    const result = await chatWithTravelAssistant(nextMessages, extraContext)
    const answer = result.answer || ''
    assistantMessage.content = answer
    assistantMessage.sources = Array.isArray(result.sources) ? result.sources : []
    assistantMessage.plan = result.plan || null
