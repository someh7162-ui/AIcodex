<template>
  <view class="container">
    ...
    <HikingBottomControls
      ...
      :offline-kit-action-text="offlineKitActionText"
      ...
      @open-offline-kit="openOfflineKit"
    />
    ...
    <HikingOfflineKitSheet
      :visible="offlineKitVisible"
      :kit="offlineKit"
      :readiness="offlineKitReadiness"
      @close="offlineKitVisible = false"
      @refresh="refreshOfflineKitSnapshot(true)"
      @copy="handleCopyOfflineKitCard"
      @toggle-check="handleToggleOfflineChecklistItem"
      @open-safety-plan="openSafetyPlanFromOfflineKit"
      @offline-pack-action="handleOfflinePackAction"
    />
  </view>
</template>
...
import HikingOfflineKitSheet from './components/HikingOfflineKitSheet.vue'
...
import {
  buildOfflineEmergencyCard,
  buildOfflineKitSnapshot,
  evaluateOfflineKitReadiness,
  getHikingOfflineKit,
  toggleOfflineChecklistItem,
  updateOfflineKitSnapshot,
} from '../../common/hiking-offline-kit'
...
const offlineKit = ref(getHikingOfflineKit())
const offlineKitVisible = ref(false)
...
const offlineKitReadiness = computed(() => evaluateOfflineKitReadiness(offlineKit.value))
const offlineKitActionText = computed(() => {
  const readiness = offlineKitReadiness.value
  return readiness.allReady ? '离线包已备' : `离线包 ${readiness.readyCount}/${readiness.totalCount}`
})
...
onLoad(async () => {
  ...
  refreshOfflineKitSnapshot(false)
})

onShow(() => {
  safetyPlan.value = getHikingSafetyPlan()
  offlineKit.value = getHikingOfflineKit()
  refreshOfflineKitSnapshot(false)
})
...
function handleEmergencySms() {
  ...
  const fallbackLocation = offlineKit.value?.snapshot?.location || null
  if (!currentLocation.value && !fallbackLocation && !safetyPlan.value.active) {
    uni.showToast({
      title: '请先等待定位成功',
      icon: 'none',
    })
  ...
  const body = buildEmergencySmsBody(currentLocation.value || fallbackLocation)
...
function openOfflineKit() {
  refreshOfflineKitSnapshot(false)
  offlineKitVisible.value = true
}

function openSafetyPlanFromOfflineKit() {
  offlineKitVisible.value = false
  openSafetyPlan()
}

function refreshOfflineKitSnapshot(showToast = false) {
  const snapshot = buildOfflineKitSnapshot({
    location: currentLocation.value,
    trackPoints: trackPoints.value,
    safetyPlan: safetyPlan.value,
    offlinePack: offlinePackRecord.value,
    networkOnline: networkOnline.value,
    compassText: compassText.value,
  })
  offlineKit.value = updateOfflineKitSnapshot(snapshot)
  if (showToast) {
    uni.showToast({
      title: '离线快照已保存',
      icon: 'none',
  ...
  return offlineKit.value
}

function handleToggleOfflineChecklistItem(itemId, checked) {
  offlineKit.value = toggleOfflineChecklistItem(itemId, checked)
}

function handleCopyOfflineKitCard() {
  refreshOfflineKitSnapshot(false)
  const body = buildOfflineEmergencyCard(offlineKit.value)
...
function showOfflineKitCard(body) {
  uni.showModal({
    title: '离线安全卡',
    content: body,
    confirmText: '知道了',
